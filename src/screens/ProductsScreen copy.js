...
const ProductsScreen = () => {
  ...
  // Determine grid columns based on screen size - optimized for desktop
  const getGridColumns = () => {
    if (isMobile) return 6; // 2 columns
    if (isTablet) return 4; // 3 columns
    if (isLargeScreen) return 2.4; // 5 columns
    if (isExtraLargeScreen) return 2; // 6 columns
    return 3; // 4 columns by default for regular desktop
  };

  // Get container width based on screen size
  const getContainerMaxWidth = () => {
    if (isExtraLargeScreen) return '1800px';
    if (isLargeScreen) return '1600px';
    if (isDesktop) return '1400px';
    return 'lg'; // Default MUI container size
  };

  // Get horizontal scroll items count based on screen size
  const getScrollItemsCount = () => {
